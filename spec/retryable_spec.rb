...
RSpec.describe 'Retryable.retryable' do
  ...
  it 'catch StandardError only by default' do
    expect do
      count_retryable(tries: 2) { |tries| raise Exception if tries < 1 }
  ...
  it 'retries on default exception' do
    ...
    count_retryable(tries: 2) { |tries| raise StandardError if tries < 1 }
  ...
  it 'makes another try if exception is covered by :on' do
    ...
    count_retryable(on: [StandardError, ArgumentError, RuntimeError]) do |tries|
  ...
  it 'does not try on unexpected exception' do
    allow(Kernel).to receive(:sleep)
    expect do
      count_retryable(on: RuntimeError) { |tries| raise StandardError if tries < 1 }
  ...
  it 'retries three times' do
    allow(Kernel).to receive(:sleep)
    count_retryable(tries: 3) { |tries| raise StandardError if tries < 2 }
  ...
  it 'retries infinitely' do
    expect do
      Timeout.timeout(3) do
        count_retryable(tries: :infinite, sleep: 0.1) { raise StandardError }
      end
  ...
  it 'executes exponential backoff scheme for :sleep option' do
    ...
    expect do
      Retryable.retryable(tries: 5, sleep: ->(n) { 4**n }) { raise RangeError }
  ...
  it 'calls :sleep_method option' do
    ...
    expect do
      Retryable.retryable(tries: 3, sleep_method: sleep_method) { |tries| raise RangeError if tries < 9 }
    end.to raise_error RangeError
  end
  ...
  it 'catches an exception that matches the regex' do
    expect(Kernel).to receive(:sleep).once.with(1)
    count_retryable(matching: /IO timeout/) { |c, _e| raise 'yo, IO timeout!' if c == 0 }
    expect(@try_count).to eq(2)
  end
  ...
  it 'accepts a callback to run after an exception is rescued' do
    expect do
      Retryable.retryable(sleep: 0, exception_cb: proc { |e| @raised = e.to_s }) do |tries|
  ...
  it 'does not retry on :not exception' do
    expect do
      count_retryable(not: RuntimeError) { |tries| raise RuntimeError if tries < 1 }
    end.to raise_error RuntimeError
    expect(@try_count).to eq(1)
  end
  ...
  it 'gives precidence for :not over :on' do
    expect do
      count_retryable(sleep: 0, tries: 3, on: StandardError, not: IndexError) do |tries|
